...
#define  ZEND_MM_BINS_INFO(_, x, y) \
	...
	_(29, 3072,    4, 3, x, y)
```
 - 思考：给一个size，如何确定它的bin_num（也就是索引），来快速找到count和pages？
```c
...
```
 - 当size <= 64的时候，size会递增8，那么直接>>3位（除以8）就可以得到索引的偏移量，很好理解。这里的!!size是为了兼容size = 0的情况，这样在右移3位之前就不会出现负值；减去!!size（即减1）是为了兼容边界情况。
 - 当size > 64的时候，具体请看[small内存规格的计算](https://segmentfault.com/a/1190000018260140) 核心思想：可以将size看成等差数列，按照公差分组。首先找出当前size属于哪个组，在组内的偏移量是多少，最终把二者相加
 - small内存分配流程:
```c
