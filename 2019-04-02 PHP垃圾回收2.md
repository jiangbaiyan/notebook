...
ZEND_API int zend_gc_collect_cycles(void)
{
		...
		//把垃圾回收器中为白色GC_WHITE的垃圾放到to_free垃圾链表中
...
```
 - 我们可以将这个函数逻辑分为几大块：
	 - 调用函数gc_mark_roots();首先进行深度优先遍历，将refcount-1，并标记为灰色
	 - 调用函数gc_scan_roots();再次进行深度优先遍历，判断当前的refcount，如果大于0，就不是垃圾，标记为黑色，并且要将refcount+1恢复到原来的值；如果等于0，就是垃圾，标记为白色
	 - 调用函数gc_collect_roots()，将白色的垃圾摘出来放进一个链表中，等待回收
