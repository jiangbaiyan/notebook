...
$a = 2;
```
 - 根据以上测试用例，在zend_execute处打一个断点，首先我们先观察传入的参数op_array，它是经过AST遍历之后生成的最终的op_array，顺便复习一下op_array中指令的存储情况：
...
![](http://baiyanzzz.oss-cn-beijing.aliyuncs.com/2019/6/4/1559611299855.png)
 - 这样一来，我们就可以清晰地看出来，每个变量、常量是在op_array中的哪部分进行存储的，现在，我们需要将其加载到虚拟机的执行栈桢上，来最终执行这些指令：
