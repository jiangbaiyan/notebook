...
```
 ...
 - 在PHP中，children结点的个数是按照如下分类存储的，如下：
```c
enum _zend_ast_kind {
	/* special nodes 特殊结点*/ 
	ZEND_AST_ZVAL = 1 << ZEND_AST_SPECIAL_SHIFT, //（1<<6 = 64）
	ZEND_AST_ZNODE, // (enum中下一个字段自动++，64 +1  = 65)

	/* declaration nodes 定义结点*/
	ZEND_AST_FUNC_DECL,
	ZEND_AST_CLOSURE,
	ZEND_AST_METHOD,
	ZEND_AST_CLASS,
...
```
 ...
 - 这样，在基本的_zend_ast结构体中，kind字段只需要存一个数字，代表它是什么类型，是LIST、0 child nodes、还是1 child nodes类型等等。如果是LIST类型，PHP会根据kind的数字值直接去找对应的结构体，对应的结构体中存有具体的children个数。而0 child等类型已经直接说明了0个、1个子结点了。
...
```
 - 利用gdb调试这段代码：
 - 是在zend_compile处打一个断点，这里会进行词法分析和语法分析（注意词法分析和语法分析是同时执行的，解析出一个token就开始进行语法分析，而不是串行执行的），并查看compiler_globals.ast字段：
