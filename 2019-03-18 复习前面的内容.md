...
```
 ...
 - 在第一个echo处打断点，并查看$a中zend_stinrg中的gc.refcount = 1(下简称refcount)。因为现在只有一个$a引用zend_string。
...
![](http://pq370w15r.bkt.clouddn.com/notebook/2019/4/23/1556020384803.png)
 - 此时的内存结构如图所示：
![](http://pq370w15r.bkt.clouddn.com/notebook/2019/4/23/1556022986659.png)
 - 这样做的优点就是仅仅需要1个zend_string就可以存储两个PHP变量的值，而不是2个zend_string，节省了内存空间。
...
```
 ...
 - 我们画一下最终$a与$b的内存结构图：
![](http://pq370w15r.bkt.clouddn.com/notebook/2019/4/23/1556022956479.png)
...
![](http://pq370w15r.bkt.clouddn.com/notebook/2019/4/23/1556023298961.png)
 - PHP7数组基本结构：
```c
struct _zend_array {
...
```
 - 此结构在内存中的结构图如下：
![](http://pq370w15r.bkt.clouddn.com/notebook/2019/4/23/1556025340931.png)
