...
```
 - 主要关注\*arData字段，数组是由bucket实现的，每一个bucket中存了一个key-value对，还有一个冗余的哈希值：
```c
...
![](http://pq370w15r.bkt.clouddn.com/notebook/2019/4/21/1555811107804.png)
     - $a['s'] = 3; 这里注意需要先修改索引数组，保证索引数组中第一个指向的bucket数组单元是最后插入bucket数组的值（头插法），并且修改val.u2.next指针，指向上一个具有相同hash值的元素。
