...
void zend_compile_assign(znode *result, zend_ast *ast)
{
	...
	switch (var_ast->kind) {
		...
		case ZEND_AST_STATIC_PROP:
			offset = zend_delayed_compile_begin(); //初始化zend_stack，并返回当前栈顶的位置（即结果及操作数在栈桢中存放的起始位置）
			zend_delayed_compile_var(&var_node, var_ast, BP_VAR_W); //处理左边的$a
			zend_compile_expr(&expr_node, expr_ast);//处理右边1
...
```
 - 首先取出赋值等号（517）的第一个子结点，其类型是ZEND_AST_VAR（256），然后取出第二个子结点，其类型是ZEND_AST_ZVAL（64）
### zend_stack初始化
 - 接下来调用zend_delayed_compile_begin():
```c
...
```
 - 它会获取delayed_oplines_stack栈顶的位置，那么这个栈的结构是怎样的呢：
```c 
...
```
 - 由于具体的元素是void* 指针，我们不知道每一个元素的长度，所以必然要有一个size字段存储栈的大小
 - 这个函数返回当前栈顶的位置，这个位置就是结果及操作数在栈桢中存放的起始位置
## 处理等号左边的结果result（即$a）
 - 接下来调用zend_delayed_compile_var(&var_node, var_ast, BP_VAR_W); 这个函数被用来编译左边的$a：
```c
...
```
 ...
 - 第二行，给result中的u.op.var字段赋值，而result我们讲过，是一个znode。重点关注右边lookup_cv()函数，它返回一个int类型的地址，是sizeof(zval)的整数倍，通过它可以得到每个变量的偏移量（80（后面会讲） + 16 * i），i是变量的索引，从而在zend_op_array（也就是栈）上方便地存储了$a这个变量（下一篇笔记会详细讲）
```c
...
```
### 编译等号右边的表达式（即1）
 - 接下来回到外部zend_compile_assign函数，继续往下执行zend_compile_expr(&expr_node, expr_ast)函数，处理等号右边的值1：
...
### 编译后续处理
 - 接下来回到外部zend_compile_assign函数，继续往下执行zend_delayed_compile_end(offset)这个函数：
 ```c
