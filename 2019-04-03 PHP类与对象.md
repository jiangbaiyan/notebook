...
## 方法的存储
 - 由于方法也属于类作用域，所有对象共享同样的方法体。所以在类结构中，就可直接以一个hashtable存储方法。key为方法名称，value为具体的zend_function：
```c
struct _zend_class_entry {
    ...
...
> 	 - 取出当前类结构中的Hashtable property_info字段，这个字段是一个哈希表，存有属性的信息。
> 	 - 将要查找的属性名作为key，到哈希表中找到对应的value，即zend_property_info结构体，并取出结构体中的offset字段
> 	 - 到当前对象zend_object结构体中，通过内存计算（柔性数组的起始地址+offset）就可以得到所要访问的当前对象的某个普通属性的值
> 	 - 如果以上都没有找到，说明它是一个动态属性，那么就去zend_object下的properties哈希表中